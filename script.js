...
async function ensureFolderExists(path){
 const listRes=await fetch("https://api.dropboxapi.com/2/files/list_folder",{method:"POST",headers:{"Authorization":"Bearer "+DROPBOX_TOKEN,"Content-Type":"application/json"},body:JSON.stringify({path})});
 if(listRes.ok) return;
 const errorData=await listRes.json();
 const errorTag=errorData?.error?.['.tag'];
 const pathErrorTag=errorData?.error?.path?.['.tag'];
 if(errorTag==="path" && pathErrorTag==="not_found"){
  await fetch("https://api.dropboxapi.com/2/files/create_folder_v2",{method:"POST",headers:{"Authorization":"Bearer "+DROPBOX_TOKEN,"Content-Type":"application/json"},body:JSON.stringify({path,autorename:false})});
  return;
 }
 throw new Error(errorData.error_summary||"Dropbox folder error");
}

async function caricaUltimi(){
 await ensureFolderExists(CARTELLA);
